...
#define CALC_INTR512_OPT
#ifdef CALC_INTR512_OPT
    const int INTR_SIZE_BYTES   = 16;
    __m512  max_dist_quad       = _mm512_set1_ps(MAX_DISTANCE_QUAD);
    __m512i ones                = _mm512_set1_epi32(1);
    __m512  x_add               = _mm512_set_ps(15.f, 14.f, 13.f, 12.f, 11.f, 10.f, 9.f, 8.f,
    ...
    #define CALC_AND_ADD_CODE(__code__)                                                                         \
        ...
        {                                                                                                       \
            float yf = (float) y / data->window_width * data->zoom + data->y_shift;                             \
            __m512 y0 = _mm512_set1_ps(yf);                                                                     \
                                                                                                                \
            for (unsigned int x = 0; x < data->window_height; x += INTR_SIZE_BYTES)                             \
            {                                                                                                   \
                float xf = (float) x;                                                                           \
                __m512  x0 = _mm512_add_ps(_mm512_set1_ps(xf), x_add);                                          \
                        x0 = _mm512_div_ps(x0, _mm512_set1_ps((float) data->window_height));                    \
                        x0 = _mm512_mul_ps(x0, _mm512_set1_ps(data->zoom));                                     \
                        x0 = _mm512_add_ps(x0, _mm512_set1_ps(data->x_shift));                                  \
                                                                                                                \
                __m512 x1 = x0;                                                                                 \
                __m512 y1 = y0;                                                                                 \
                                                                                                                \
                ...
                for (int i = 0; i < MAX_DOT_TRIES; ++i)                                                         \
                {                                                                                               \
                    __m512 x1_quad = _mm512_mul_ps(x1, x1);                                                     \
                    __m512 y1_quad = _mm512_mul_ps(y1, y1);                                                     \
                    __m512 x1_y1   = _mm512_mul_ps(x1, y1);                                                     \
                                                                                                                \
                    __m512 x1q_plus_y1q = _mm512_add_ps(x1_quad, y1_quad);                                      \
                    __m512 x1_sub_x2    = _mm512_sub_ps(x1_quad, y1_quad);                                      \
                    __m512 dx1_y1       = _mm512_add_ps(x1_y1,   x1_y1  );                                      \
                                                                                                                \
                    __mmask16 cmp_mask = _mm512_cmp_ps_mask(x1q_plus_y1q, max_dist_quad, _CMP_LT_OQ);           \
                    if (!cmp_mask)                                                                              \
                        break;                                                                                  \
                                                                                                                \
                    iters = _mm512_mask_add_epi32(iters, cmp_mask, iters, ones);                                \
                                                                                                                \
                    x1 = _mm512_add_ps(x1_sub_x2, x0);                                                          \
                    y1 = _mm512_add_ps(dx1_y1,    y0);                                                          \
                }                                                                                               \
                                                                                                                \
                __code__                                                                                        \
            }                                                                                                   \
    ...
    void CalcMandelbrotWithColors(MandelbrotInfo* data)
    {
        CALC_AND_ADD_CODE({memcpy(data->arr_iters + y*data->window_width + x, &iters, sizeof(__m512i));})
    }
#endif //CALC_INTR512_OPT
